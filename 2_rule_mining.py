def load_data_set(fname):
    l = []
    file_iter = open(fname, 'rU')
    for line in file_iter:
        line = line.strip().rstrip(',')
        s = line.split(',')
        while '' in s:
            s.remove('')
        l.append(s)
    return l


def create_C1(data_set):
    C1 = set()
    for t in data_set:
        for item in t:
            item_set = frozenset([item])
            C1.add(item_set)
    return C1

def create_Ck(Lksub1, k):
    Ck = set()
    len_Lksub1 = len(Lksub1)
    list_Lksub1 = list(Lksub1)
    for i in range(len_Lksub1):
        for j in range(1, len_Lksub1):
            l1 = list(list_Lksub1[i])
            l2 = list(list_Lksub1[j])
            l1.sort()
            l2.sort()
            if l1[0:k-2] == l2[0:k-2]:
                Ck_item = list_Lksub1[i] | list_Lksub1[j]
                if Apriori(Ck_item, Lksub1):
                    Ck.add(Ck_item)
    return Ck

def Apriori(Ck_item, Lksub1):
    for item in Ck_item:
        sub_Ck = Ck_item - frozenset([item])
        if sub_Ck not in Lksub1:
            return False
    return True

def generate_Lk_by_Ck(data_set, Ck, min_support, support_data):
    Lk = set()
    item_count = {}
    for t in data_set:
        for item in Ck:
            if item.issubset(t):
                if item not in item_count:
                    item_count[item] = 1
                else:
                    item_count[item] += 1
    t_num = float(len(data_set))
    for item in item_count:
        if (item_count[item] / t_num) >= min_support:
            support_data[item] = item_count[item] / t_num
            Lk.add(item)
    return Lk

def generate_L(data_set, k, min_support):
    support_data = {}
    C1 = create_C1(data_set)
    L1 = generate_Lk_by_Ck(data_set, C1, min_support, support_data)
    Lksub1 = L1.copy()
    L = []
    L.append(Lksub1)
    
    for i in range(2, k+1):
        Ci = create_Ck(Lksub1, i)
        Li = generate_Lk_by_Ck(data_set, Ci, min_support, support_data)
        Lksub1 = Li.copy()
        L.append(Lksub1)
    return L, support_data


def generate_big_rules(L, support_data, min_conf):
    big_rule_list = []
    sub_set_list = []
    for i in range(0, len(L)):
        for freq_set in L[i]:
            for sub_set in sub_set_list:
                if sub_set.issubset(freq_set):
                    conf = support_data[freq_set] / support_data[freq_set - sub_set]
                    big_rule = ((freq_set - sub_set, sub_set), support_data[freq_set],  conf, conf / support_data[sub_set])
                    if conf >= min_conf and big_rule not in big_rule_list:
                        big_rule_list.append(big_rule)
            sub_set_list.append(freq_set)
    return big_rule_list


def saveResults(items, rules):
    fre_items_file = open(r'fre_items.txt', 'w')
    confidence_order = open(r'confidence_order.txt', 'w')
    rules_order_by_lift = open(r'lift_order.txt', 'w')
    fre_items_file.write('frequent_item_set\tsupport\n')
    
    # support
    for item, support in sorted(items, key=lambda (item, support): support, reverse=True):
        fre_items_file.write("%s\t%.3f\n" % (str(item), support))
    fre_items_file.close()

    # confidence
    confidence_order.write('rule\tsupport\tconfidence\tlift\n')
    for rule, support, confidence, lift in sorted(rules, key=lambda (rule, suppport, confidence, lift): confidence, reverse=True):
        pre, post = rule
        confidence_order.write("%s ==> %s\t%.3f\t%.3f\t%.3f\n" % (str(pre), str(post), support, confidence, lift))
    confidence_order.close()

    # lift
    lift_order.write('rule\tsupport\tconfidence\tlift\n')
    for rule, support, confidence, lift in sorted(rules, key=lambda (rule, suppport, confidence, lift): lift, reverse=True):
        pre, post = rule
        lift_order.write("%s ==> %s\t%.3f\t%.3f\t%.3f\n" % (str(pre), str(post), support, confidence, lift))
    lift_order.close()


if __name__ == "__main__":
    data_set = load_data_set('Pre_data.csv')
    L, support_data = generate_L(data_set, k=3, min_support=0.2)
    big_rules_list = generate_big_rules(L, support_data, min_conf=0.7)
    l = []
    for Lk in L:
        for freq_set in Lk:
            l.append((freq_set, support_data[freq_set])) 
    saveResults(l, big_rules_list) 